$(document).ready(function() {
    ...
    function addskill() {  
        ...
        $('.skills-loader').show();
        var skill_val=$( ".skills:checked" ) .map(function() {   return this.value;  }) .get(); 
       /* if(skill_val.length > 3){
        ...
        $.ajax({
            ...
            type: 'POST',
            dataType: 'json',
            data: { '_token': token, 'skills':skills, 'totalskills': totalskills  },
            success: function(data) {
               $('.skills-loader').hide();
            },
            error: function(data) {
...
$('.submitCertificate').click(function(){
    ...
    certificateVerification.submit(function(e){
        ...
        $.ajax({
            ...
            processData: false,
            success:function(data){
                $('#veri-certificate-popup').slideUp();
                console.log(window.location.href =  url + '/verification');
            },
            error: function (data) {
                var obj = jQuery.parseJSON( data.responseText );
    ...
    $('#mytask-image-upload').click(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                $('#general-ajax-load').hide();
                $.each(data, function(index, data) {
                    $('.taskimages').append('<span><img class="my_task_attach_pic"   src="public/taskdocs/' + data.name + '"  style="width:75px" /><i class="fa cut fa-times-circle delete-tf" aria-hidden="true" data-id="' + data.tid + '"></i> </span>');
                });
            }
        });
    });
...
$(document).ready(function() {
    ...
    $('.task-submission').click(function(e) {
        ...
        $.ajax({
            url: url + '/regsitercomplain-userpanel',
            type: 'POST',
            dataType: 'json',
            data: data,
            success: function(data) {
